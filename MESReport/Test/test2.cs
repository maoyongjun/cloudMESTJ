...
{
    public class HWTTestlianglvReport1 : ReportBase
    {
...
and b.REPAIRDATE=to_char(sysdate,'yyyy-mm-dd') and a.FAILCATEGORY='DEFECT' group by a.FAILCODE order by 2 desc";
            ...
            for (int i = 0; i < data1.Tables[0].Rows.Count; i++)
            {
                ...
                int j = i + 1;
                C.center = new List<string>() { $@"{j * 10 + 5}%", $@"50%" };
            ...
            for (int i = 0; i < data2.Tables[0].Rows.Count; i++)
            {
                ...
                double v = double.Parse(data2.Tables[0].Rows[i]["ICT_FPY"].ToString());
                bardata1.data.Add(v);
            }
            ...
            Chart3.legend = new
            {
                data = new List<string>() {"PASS","FAIL" }
            };
            Chart3.yAxis = yAxis;
            ...
            for (int i = 0; i < data2.Tables[0].Rows.Count; i++)
            {
                lines2.Add(data2.Tables[0].Rows[i]["PRODUCTIONLINE"].ToString());
                double v = double.Parse(data2.Tables[0].Rows[i]["ICT_FPY"].ToString());
                bardata2.data.Add(v);
            }
