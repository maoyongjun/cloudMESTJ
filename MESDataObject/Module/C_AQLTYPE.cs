...
namespace MESDataObject.Module
{
    public class T_C_AQLTYPE : DataObjectTable
    {
        ...
        public List<C_AQLTYPE> GetAllAql(OleExec DB)
        {
            ...
            {
                sql = $@" select * from c_aqltype order by aql_type,gl_level,lot_qty asc ";
        ...
        public List<string> GetAqlLevelByType(string aqltype, OleExec DB)
        {
            ...
            string strSql = $@" SELECT DISTINCT GL_LEVEL FROM C_AQLTYPE where AQL_TYPE='{aqltype}'  ";
            DataTable dt = DB.ExecSelect(strSql).Tables[0];
            foreach (DataRow VARIABLE in dt.Rows)
                res.Add(VARIABLE[0].ToString());
            return res;
        }
        ...
        public List<C_AQLTYPE> GetAqlTypeBySkunoAndLevel(string skuno,string AqlLevel, OleExec DB)
        {
            ...
            if (this.DBType.Equals(DB_TYPE_ENUM.Oracle))
            {
                sql = $@" select b.* from  c_sku_aql a,c_aqltype b where a.skuno='{skuno}' and a.aqltype=b.aql_type and b.gl_level='{AqlLevel}' ";
        ...
        public List<C_AQLTYPE> GetAqlTypeBySkunoId(string skuId, OleExec DB)
        {
            ...
            if (this.DBType.Equals(DB_TYPE_ENUM.Oracle))
            {
                sql = $@"  select b.* from c_sku a,c_aqltype b,c_sku_aql c where a.skuno=c.skuno and b.aql_TYPE=C.AQLTYPE  and a.id='{skuId}'  ";
                dt = DB.ExecSelect(sql, null).Tables[0];
                foreach (DataRow dr in dt.Rows)
                {
                    aqlsRow.loadData(dr);
