...
var ReportInput = function (obj) {
    ...
    ReportInput.prototype.Show = function (obj) {
        var E = new InputElements(obj.Client, this.EnterSubmit);
...
var ReportOutput = function (obj) {
    ...
    this.Tittle = obj.Tittle || obj.title.text;
...
var InputElements = function (client, AutoPostback) {
    this.client = client;
    ...
    InputElements.prototype.Text = function (c, ID, Label, placeholder, value, RefershType, Scale, ScanFlag) {
        ...
        var input = $("<input id=\"" + ID + "\" name=\"" + Label + "\"  type=\"text\" class=\"form-control\" placeholder=\"" + placeholder + "\" value=\"" + value + "\" " + (this.AutoPostback ? "data-AutoPostback='True'" : "data-AutoPostback='False'") + ">");
        inputD.append(input);
    ...
    InputElements.prototype.Select = function (c, ID, Label, placeholder, value, DataForUse, Scale) {
        ...
        var inputD = $("<div class=\"col-xs-" + scales[1] + "\"></div>");
        var input = $("<select id=\"" + ID + "\" name=\"" + Label + "\" class=\"form-control\" placeholder=\"" + placeholder + "\" aria-describedby=\"basic-addon1\" " + (this.AutoPostback ? "data-AutoPostback='True'" : "data-AutoPostback='False'") + "></select>");
        inputD.append(input);
        div.append(label, inputD);
        c.append(div);
        var select = $("#" + ID);
    ...
    InputElements.prototype.Checkbox = function (c, ID, Label, placeholder, value, RefershType, Scale) {
        ...
        var label = $("<label for=\"" + ID + "\" class=\"col-xs-" + scales[0] + " control-label text-right\">" + Label + "</label>");
        var inputD = $("<div class=\"col-xs-" + scales[1] + "\"></div>");
        var input = $("<input id=\"" + ID + "\" name=\"" + Label + "\"  type=\"checkbox\" class=\"form-control\"" + (value ? "checked" : "") + (this.AutoPostback ? "data-AutoPostback='True'" : "data-AutoPostback='False'") + ">");
        inputD.append(input);
        div.append(label, inputD);
        c.append(div);
    };
    InputElements.prototype.Radio = function (c, ID, Label, placeholder, value, DataForUse, API, APIPara, RefershType, Scale) {
        ...
        for (var i = 0; i < DataForUse.length; i++) {
            ...
            var radio = $("<input type=\"radio\" name=\"" + Label + "\" id=\"" + ID + "_" + i + "\"" + (value == DataForUse[i] ? "checked" : "") + (this.AutoPostback ? "data-AutoPostback='True'" : "data-AutoPostback='False'") + ">");
...
var OutputElements = function () {
    ...
    OutputElements.prototype.EChart = function (c, ID, ChartData) {
        c.empty();
        var d = $("<div style=\"height:100%\" view-group=\"" + ID + "\"></div>");
        var elem;
        if (ChartData.Zone_ID) {
            var e = $("#" + ChartData.Zone_ID);
            e.empty();
            e.append(d);
            if (e.length > 0) {
                elem = d[0];
