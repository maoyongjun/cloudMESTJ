...
namespace MESInterface.HWD
{
    public class AssyReturnBackFlush : taskBase
    {
        ...
        private void AssyToMrbBackFlush()
        {
            ...
            sql = $@" update r_mrb_gt  set sap_flag = '3' where (sap_flag = 0 or sap_flag='2') and zcpp_flag = 1  and sap_station_code is not null ";
            SFCDB.ExecSQL(sql);

            sql = $@" select * from r_mrb_gt where sap_flag = 3 and zcpp_flag = 1  and sap_station_code is not null ";
