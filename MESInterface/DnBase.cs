...
namespace MESInterface
{
    public class DnBase 
    {
        ...
        public SAP_RFC.ZRFC_SFC_NSG_0003 ZRFC_SFC_NSG_0003;
        ...
        public DnBase(string bu,string plant,string dbStr,string cust)
        {
            Bu = bu; Plant = plant; DbStr = dbStr; Cust = cust;
            try
            {
                ZRFC_SFC_NSG_0003 = new SAP_RFC.ZRFC_SFC_NSG_0003(Bu);
            }
            catch (Exception ee)
            {
                throw ee;
        ...
        public void CallRfc()
        {
            ...
            {
                ZRFC_SFC_NSG_0003.SetValue(DnCreatDateTime, this.Cust, this.Plant, "");
            }
            else
            {
                ZRFC_SFC_NSG_0003.SetValue(DateTime.Now.ToString("yyyy-MM-dd"), this.Cust, this.Plant, "");
            }
        ...
        void RfcDataToModelList()
        {
            ...
            if (ZRFC_SFC_NSG_0003.GetTableValue("SD_TO_HEAD").Rows.Count == 0)
                return;
            sdCustmerPoList = MESDataObject.DataObjectTable.ConvertToEx<SD_CUSTMER_PO>(ZRFC_SFC_NSG_0003.GetTableValue("SD_CUSTOMER_PO"))?.Distinct().ToList();
            sdCustmerPoItemList = MESDataObject.DataObjectTable.ConvertToEx<SD_CUSTOMER_PO_ITEM>(ZRFC_SFC_NSG_0003.GetTableValue("SD_CUSTOMER_PO_ITEM"))?.Distinct().ToList();
            sdCustmerSoList = MESDataObject.DataObjectTable.ConvertToEx<SD_CUSTOMER_SO>(ZRFC_SFC_NSG_0003.GetTableValue("SD_CUSTOMER_SO"))?.Distinct().ToList();
            sdDnDetailList = MESDataObject.DataObjectTable.ConvertToEx<SD_DN_DETAIL>(ZRFC_SFC_NSG_0003.GetTableValue("SD_DN_DETAIL"))?.Distinct().ToList();
            sdReportDetailList = MESDataObject.DataObjectTable.ConvertToEx<SD_REPORT_DETAIL>(ZRFC_SFC_NSG_0003.GetTableValue("SD_REPORT_DETAIL"))?.Distinct().ToList();
            sdToDetailList = MESDataObject.DataObjectTable.ConvertToEx<SD_TO_DETAIL>(ZRFC_SFC_NSG_0003.GetTableValue("SD_TO_DETAIL"))?.Distinct().ToList();
            sdToHeadList = MESDataObject.DataObjectTable.ConvertToEx<SD_TO_HEAD>(ZRFC_SFC_NSG_0003.GetTableValue("SD_TO_HEAD"))?.Distinct().ToList();
        }
